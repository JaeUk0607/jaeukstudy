			...
			<table width="80%" border="1" cellspacing="0" align="center">
				...
				<c:forEach var="board" items="<%=list%>">
					...
					<tr onmouseover="this.style.backgroundColor='gray'" onmouseout="this.style.backgroundColor='white'">
						...
						<td align="left">
							...
							<a href='content.bbs?idx=${idx}&cp=${cpage}&ps=${pagesize}'>
								<c:choose>
				...
				<tr>
					...
					-->
						...
						<!--이전 링크 --> 
						<c:if test="${cpage>1}">
							<a href="list.bbs?cp=${cpage-1}&ps=${pagesize}">이전</a>
							<!--페이지 리스트 구현  -->
						</c:if> 
						<c:forEach var="i" begin="1" end="${pagecount}" step="1">
							<c:choose>
								...
								<c:otherwise>
									<a href="list.bbs?cp=${i}&ps=${pagesize}">[${i}]</a>
								</c:otherwise>
							</c:choose>
						</c:forEach> 
						<!--다음 링크 --> 
						<c:if test="${cpage<pagecount}">
							<a href="list.bbs?cp=${cpage+1}&ps=${pagesize}">다음</a>
						</c:if>
					</td>
