...
public class NewArticleController {
	...
	//전제 조건 : Form 태그의 name 속성의 이름이 DTO 객체의 memberfield와 같아야한다.
	//<input type="text" name="title"> >> private String title;
	
	//submit (NewArticleCommend commend) 함수의 parameter DTO 타입을 사용
	//넘어오는 parameter 가 DTO 타입의 memberfield 명과 같다면
	//1. 자동으로 DTO 객체 생성 : NewArticleCommand  newArticleCommand = new New NewArticleCommand)_;
	...
	//위 원칙이 없어도 view 페이지에 DTO 객체 (NewArticleCommend) 주소가 자동 forward....
	...
	public String submit(@ModelAttribute("Articledata") NewArticleCommand command) {
		//1. parameter 받기
		//2. Service 객체 생성하기 New ArticleService service = new ArticleService()
