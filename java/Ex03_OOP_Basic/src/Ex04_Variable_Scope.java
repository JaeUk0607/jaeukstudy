...
class Variable{
	...
	2. 목적 : 정보를 담을려고 한다(고유데이터, 상태데이터, 참조데이터) >> 생성되는 객체마다 다른 값을 갖기 위해서
	3. 특징 : 초기값 가지고 있다 (각각의 타입이 정의하는 : int >0, float > 0.0, boolean > false, String 또는 참조값의 기본값 > null
	4. 생성시점(memory를 갖는 시점) : new 라는 연산자를 통해서 heep 객체가 만들어 지고 나서
	*/
	...
	/*
	 ...
	 3. 특징 : 접근방법 >> 클래스 이름.static자원 (객체가 만들어 지지 않은 상태에서 접근가능) : Math.Random()
	                                           또는 참조변수.static 자원( variable v = new variable(); .........v.cv)
	 4. 생성시점 : 프로그램 실행(class loader 의 의해서) 클래스 정보를 읽어서 class area, static area 올리고 나서
	 	이떄 클래스가 static 자원을 가지고 있다면 static 자원도 같이 memory 올려요
